...
function ImageParticle(img, posx, posy) {
	...
	this.update = function() {
		...
		var velX = this.vel * Math.cos(this.theta );
		
		var velY;
		if(this.landCheck){
			velY = Math.min(this.vel * Math.sin(this.theta ) + this.gravity, 3);
		}else{
			velY = 0;
		}
		...
		//recalculate veloctity and theta;
		// console.log( Math.atan2(velY, velX) );
		this.theta = Math.atan2(velY, velX);
		this.vel = Math.sqrt( velY * velY + velX * velX );
		
		if(this.landCheck && this.posY > 120){
			this.theta = 0 ;
